...
On peut ensuite comparer la performance réalisée par l'algorithme sur les données d’entraînement et les données de tests. Se pose alors le problème de comment sélectionner ces données. En effet, les données de base pouvant potentiellement être triées (ordonnées par rapport à une des caractéristiques), sélectionner une partie pourrait créer un biais dans l'apprentissage.
